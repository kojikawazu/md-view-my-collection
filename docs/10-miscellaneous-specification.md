...
- Markdown ライブラリ調査（react-markdown / remark-gfm / rehype-sanitize）
- データ品質チェック（Reportテーブル ハルシネーション・品質チェックレポート）
...
| ファイル | 内容 |
| --- | --- |
| react-markdown | Markdown → React 変換コンポーネント（本書「react-markdown」セクション） |
| remark-gfm | GitHub Flavored Markdown 拡張プラグイン（本書「remark-gfm」セクション） |
| rehype-sanitize | HTML サニタイズプラグイン（本書「rehype-sanitize」セクション） |
