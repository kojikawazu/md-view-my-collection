...
## データ要件
- Report: `id`, `title`, `summary`, `content`, `category`, `author`, `date`, `tags`
- User: `id`, `username`, `role`（`admin`）
- 必須項目: title/summary/content/category/date
- タグ入力はカンマ区切り → `#` 付き配列に正規化
- DB設計は現状のままでOK。Supabase設定は別プロジェクトで実施する。
- Prismaはスキーマ取得（`prisma db pull`）のみを前提とする。
...
## セキュリティ詳細（Markdown）
- `a` は `rel="noopener noreferrer"` を付与し、`target="_blank"` を許可。
- `img` の `src` は `http/https` のみ許可。
