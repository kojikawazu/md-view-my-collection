...
- [テスト設計: repositories 共通クライアント](#テスト設計-repositories-共通クライアント)
  ...
  - [テストケース一覧](#テストケース一覧-2)
    ...
    - [準正常系](#準正常系-4)
    - [異常系](#異常系-4)
- [テスト設計: レポート応答の実行時検証（lib/report.ts）](#テスト設計-レポート応答の実行時検証libreportts)
  - [対象](#対象-3)
  - [モック方針](#モック方針-3)
  - [テストケース一覧](#テストケース一覧-3)
    - [正常系](#正常系-6)
...
- 実装ファイル: `front/tests/e2e/app.spec.ts`、`front/tests/e2e/markdown.spec.ts`（本文の Markdown 描画）
- 共有ヘルパー: `front/tests/e2e/helpers.ts`（fixture データ・setStorage を分離）
- カバー済み: TC-001 〜 TC-031, TC-035 〜 TC-042
...
- 本文のMarkdown描画
  - 前提: Markdown記法を含む本文
  - 手順: 詳細を開く
  - 期待: 記法が要素として描画され、生HTMLはサニタイズされる（TC-037〜TC-042）
- 404相当
  - 前提: 存在しないID
  - 手順: `/report/unknown`
  - 期待: “Report Not Found”
...
### 本文の Markdown 描画（詳細: `front/tests/e2e/markdown.spec.ts` / Issue #25）

詳細画面の本文がプレーンテキスト表示に戻る回帰を検知する。`ReportMarkdown` は
**標準記法の描画・独自変換・サニタイズ**の 3 つを担っており、観点ごとにケースを分ける。
独自変換とサニタイズは壊れても画面が一見正常に見えるため、目視では検知できない。

- TC-037 正常: 主要記法が Markdown として描画される
  - 前提: 主要記法を含む本文を持つレポート（fixture `md-1`）
  - 手順: `/report/md-1` を開く
  - 期待: 見出し / 強調 / イタリック / 打ち消し線 / リスト / 引用 / コードブロック（`pre > code`）が要素として描画され、リンクは `href`、画像は `src` と `alt` を保持する

- TC-038 正常: GFM のテーブルが表として描画される
  - 前提: 同上
  - 手順: `/report/md-1` を開く
  - 期待: `table` ロールで描画され、列見出しとセルが参照できる（`remark-gfm` が有効であること）

- TC-039 正常: 「・」始まりの段落が箇条書きへ変換される
  - 前提: 全行が全角中点で始まる段落を含む本文
  - 手順: `/report/md-1` を開く
  - 期待: `ul.dot-bullet-list` として描画され、項目数と各項目のテキストが一致する（`parseDotListItems` の独自変換）

- TC-040 正常: 見出し語に応じてコールアウトが付与される
  - 前提: `## まとめ` / `## 注意点` / `## 動向` を含む本文
  - 手順: `/report/md-1` を開く
  - 期待: 各セクションが `section.report-callout--summary` / `--warning` / `--trend` で囲まれる（`classifyHeading` の判定）

- TC-041 異常: 生 HTML がサニタイズされ実行されない
  - 前提: `<script>` / `onerror` 付き `img` / `javascript:` リンクを含む本文（fixture `md-2`）
  - 手順: `/report/md-2` を開く
  - 期待: いずれも DOM に注入されず、スクリプトが実行されていない（`rehype-sanitize` が機能していること）

- TC-042 準正常: 本文が未取得のときは本文を描画しない
  - 前提: `content` が空のレポート（fixture `md-3`）
  - 手順: `/report/md-3` を開く
  - 期待: タイトル等のメタ情報は表示されるが、`.report-markdown` は描画されない
...
# テスト設計: レポート応答の実行時検証（lib/report.ts）

## 対象

- 対象機能: 外部由来のレポート値（API レスポンス・localStorage）を状態へ入れる前の検証
- 対象ファイル: `front/src/lib/report.ts`
- スタック: Frontend (Next.js 16 / Vitest)
- テストファイル: `front/src/lib/__tests__/report.test.ts`

`ReportItem.category` を `ReportCategory` union として宣言できる根拠がこの検証にある（Issue #190）。
DB 側に CHECK 制約が無いため、**型の断言を実行時に裏付ける唯一の場所**として単独で検証する。

## モック方針

- モック許可: `console.error`（出力抑制。呼ばれたこと自体は検証対象）
- モック禁止: zod スキーマ（`schemas/report.ts`）。検証ロジックそのものであり実物を使う
...
| # | テストケース | 入力 | 期待結果 | 優先度 |
|---|---|---|---|---|
| R-N-1 | 全項目そろった 1 件 | API 形式の完全なレコード | そのまま返す | High |
| R-N-2 | 全件が妥当な配列 | 妥当な 2 件 | 2 件とも返す | High |

### 準正常系

| # | テストケース | 入力 | 期待結果 | 優先度 |
|---|---|---|---|---|
| R-S-1 | local モードのレコード（監査列・要約なし） | createdAt/updatedAt/summary 欠落 | 通し、`externalUrls` を空配列で補う | High |
| R-S-2 | 壊れた要素の混在 | 妥当 1 件 + カテゴリ不正 1 件 | 妥当な 1 件のみ返し、`console.error` に残す | High |
| R-S-3 | 空配列 | `[]` | `[]`（「0 件」であり異常ではない） | Medium |

### 異常系

| # | テストケース | 入力 | 期待結果 | 優先度 |
|---|---|---|---|---|
| R-A-1 | 固定リスト外のカテゴリ | category="Unknown" | `null` を返し `console.error` | High |
