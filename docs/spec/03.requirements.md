...
## 参照ドキュメント
- `docs/spec/02.ui-layout.md`（最新UIレイアウト要求）
- `docs/spec/04.e2e-cases.md`（E2Eテスト仕様・詳細ケース）
...
- 外部URL: レポートに外部URL（note / Zenn / はてなブログ等）を複数紐付けて保存・閲覧できる（詳細: `docs/spec/08.external-url.md`）。
- 認証: 管理者がログイン/ログアウトできる。

## データ要件
- Report: `id`, `title`, `summary`, `content`, `category`, `author`, `publishDate`, `tags`
- ExternalUrl: `id`, `reportId` (FK → Report.id), `url`, `label`(nullable), `createdAt` — 1 Report : N ExternalUrl（詳細: `docs/spec/08.external-url.md`）
