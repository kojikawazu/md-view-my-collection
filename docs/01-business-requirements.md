...
- [アーキテクチャ概要](#アーキテクチャ概要)
  - [保存（DB）](#保存db)
  - [閲覧（Webサイト）](#閲覧webサイト)
- [推奨DB設計（最小で強い）](#推奨db設計最小で強い)
  - [`reports`（または `entries`）テーブル例](#reportsまたは-entriesテーブル例)
- [セキュリティ（Markdownでも必須）](#セキュリティmarkdownでも必須)
  - [推奨（Next.js / React想定）](#推奨nextjs--react想定)
- [Web機能](#web機能)
  - [画面](#画面)
  - [最初の運用](#最初の運用)
- [権限設計（後から拡張しやすい）](#権限設計後から拡張しやすい)
  - [Supabase運用（想定）](#supabase運用想定)
- [取り込み（保存）方法の整理](#取り込み保存方法の整理)
  - [A. 手動クリップ（最初はこれが強い）](#a-手動クリップ最初はこれが強い)
  - [B. 自動生成（将来的）](#b-自動生成将来的)
- [次の一手（おすすめ順）](#次の一手おすすめ順)
- [メモ（なぜHTMLスライドは後回し？）](#メモなぜhtmlスライドは後回し)
