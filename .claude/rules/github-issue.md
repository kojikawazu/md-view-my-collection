...
- **起票前の重複確認（必須）**: `gh issue create` の前に **`gh issue list --state open` で open issue のタイトルを全件確認する**。キーワード検索（`--search`）だけで済ませない — 同じ作業でも表現が違えば引っかからない。
  - 既存 issue と**範囲が重なる場合は新規起票しない**。既存 issue へのコメント追記を第一候補とし、不足分だけを追記する。
  - それでも重複させてしまった場合は、**範囲が広い方を残して狭い方をクローズ**し、閉じる側にしか書かれていない情報を残す側へ転記する。
  - 根拠となった事例は [`../../docs/lessons-learned.md`](../../docs/lessons-learned.md)（2026-08-08 のエントリ）を参照。
